...
public class GraphWebhookFunction
{
    ...
    // Returns 202 immediately (Graph retries if no response within a few seconds).
    // Each messageId is enqueued so EmailProcessorFunction handles it with guaranteed delivery + retries.
    [Function("GraphWebhookNotify")]
    public async Task<NotifyOutput> Notify(
        [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "EmailWatch/notify")] HttpRequestData req)
    {
        ...
        if (!string.IsNullOrEmpty(validationToken))
        {
            ...
            return new NotifyOutput { HttpResponse = valResponse, MessageIds = [] };
        }
        ...
        if (string.IsNullOrWhiteSpace(body))
        {
            ...
            return new NotifyOutput { HttpResponse = req.CreateResponse(HttpStatusCode.Accepted), MessageIds = [] };
        }

        var messageIds = ExtractMessageIds(body);
        _logger.LogInformation("📬 Enqueuing {Count} message(s) for processing.", messageIds.Count);
    ...
    {
        [QueueOutput("email-processing", Connection = "AzureWebJobsStorage")]
    ...
    internal List<string> ExtractMessageIds(string body)
    {
        ...
        {
            _logger.LogError(ex, "❌ Failed to parse Graph notification JSON.");
            return ids;
        }
        ...
        foreach (var notification in notifications)
        {
            ...
            {
                _logger.LogWarning("⚠️ clientState mismatch — Expected: '{Expected}' | Got: '{Actual}'.",
